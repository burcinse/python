...
for i, row in df.iterrows():
    ...
    for index, url in enumerate(imgs):
        sku = row['SKU']
        dir_1 = sku[0:8] #First three digits in the first level directory
        #dir_2 = code[3:6] #The next three digits are placed in the second level directory

        if index > 0:
            filename = row['SKU'] + "__" + str(index+1) + ".jpg"
        else:
            filename = row['SKU'] + ".jpg"

        if url:
            if index > 0:
                dst_path = exe_dir + "/images/" + os.path.join(dir_1 + '__' + str(index+1), filename)
            else:
                dst_path = exe_dir + "/images/" + os.path.join(dir_1, filename)
            stored_path = exe_dir + "/images-stored/" + os.path.join(dir_1, filename)
            ...
            if flag == "True":
                ...
                if index > 0:
                    if not os.path.isdir(os.path.join(exe_dir, "images", dir_1 + '__' + str(index+1))):
                        os.mkdir(os.path.join(exe_dir, "images", dir_1 + '__' + str(index+1)))
                else:
